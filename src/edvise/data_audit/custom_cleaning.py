...
def _is_four_digit_code_series(series: pd.Series) -> bool:
    """
    True when every non-null value is a bare 4-digit token.

    Generic ``pd.to_datetime`` treats strings like ``"2021"`` as 2021-01-01; skip datetime
    inference for these columns so codes stay nullable Int64. This covers calendar year codes
    (``year_code``) and term codes outside any calendar-year range — Banner/PeopleSoft terms
    such as ``2187`` or ``2257`` encode century, year, and season, and parsing them as dates
    silently rewrites them to ``2187-01-01``.
    """
    non_null = series.dropna()
    if non_null.empty:
        return False
    tokens = non_null.astype("string").str.strip()
    return bool(tokens.str.fullmatch(r"\d{4}").all())
