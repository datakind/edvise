"""Target generation for Edvise (ES) projects — same pipeline as :mod:`edvise.scripts.pdp_targets` with :class:`edvise.configs.es.ESProjectConfig` ."""
...
class ESTargetsTask:
    """Computes the target variable for an Edvise (ES) SST pipeline run."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Target generation for Edvise (ES) SST pipeline."
    )
...
if __name__ == "__main__":
    ...
    task = ESTargetsTask(args)
    log_path = init_file_logging(
        args,
        task.cfg,
        logger_name=__name__,
        log_file_name="es_targets.log",
    )
