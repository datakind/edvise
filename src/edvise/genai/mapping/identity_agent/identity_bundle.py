"""
Single handoff bundle for IdentityAgent LLM outputs (grain + term stages).
...
class InstitutionIdentityContract(BaseModel):
    """
    One institution, one artifact: grain and term envelopes together.

    Use when persisting or passing IdentityAgent results between steps; build the SMA-facing
    frozen contract via :func:`~edvise.genai.mapping.identity_agent.execution.contract_builder.build_schema_contract_from_grain_contracts`
...
def institution_identity_contract_from_parts(
    ...
    """Build :class:`InstitutionIdentityContract` when grain and term share ``institution_id``."""
