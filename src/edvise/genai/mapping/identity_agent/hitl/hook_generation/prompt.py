...
- If reviewer_note is present, treat it as the authoritative instruction and override any draft logic in config_snippet.
- If reviewer_note is absent, use hitl_context and config_snippet together to infer the correct implementation.
- **Expression drafts from the term stage:** ``config_snippet.term_config.hook_spec`` may list **one-line expression** strings in ``functions[].draft``. Treat them as the **intended semantics**; your output must be **full** ``def`` blocks that implement the same behavior (same raw tokens, same parsing). Do not "optimize" into a shorter formula that changes which month maps to which bucket.
- **``year_extractor`` and dates:** Hooks run after ``clean_dataset`` has applied dtype generation. The ``term`` argument is the term column as **string** (``astype("string")``). Datetime columns are usually parsed to ``datetime64`` first, then stringified for the hook — often as **ISO-like** values (e.g. ``2011-04-04``), even when the raw file or profiling showed ``%m/%d/%Y`` or other layouts. **Default:** ``pd.to_datetime(term)`` with **no** fixed ``format=`` (e.g. ``int(pd.to_datetime(term).year)``), so both ISO and slash forms work. **Avoid** ``pd.to_datetime(term, format="%m/%d/%Y")`` unless you know strings stay in that form at runtime. Use an explicit ``format=`` only for strict validation or to disambiguate genuinely ambiguous dates (pair with ``dayfirst`` / ``yearfirst`` when needed).
- **Month→season from parsed dates:** Prefer **explicit** logic — ``if`` / ``elif`` on month number, a ``dict`` from month (or English month name) to raw token, or ``pd.to_datetime(term).strftime('%B').lower()`` when ``season_map`` keys are full month names. **Do not** use a single unverified arithmetic expression on month (e.g. ``(m - 1) // 4 + 1``) for policies like Jan–Apr / May–Jul / Aug–Nov / Dec — those bands are not equal-width quarters.
- **season_extractor vs season_map (single ``term_col``):** Runtime applies **two** steps: (1) your hook returns a **raw** season token (string); (2) ``term_config.season_map`` maps each ``raw`` key to the canonical label (FALL, SPRING, SUMMER, WINTER) for ``_edvise_term_season`` and ordering. The hook must **not** return canonical labels — only values that match ``season_map[].raw`` (after normalization). If ``description`` mentions a canonical season for humans, state clearly that the **return value** is still the raw token (e.g. ``'9'`` or ``'september'``), not ``'FALL'``).
