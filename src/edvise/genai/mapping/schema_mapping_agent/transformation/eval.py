...
Output folders are `{base}_2b_{SHOT_TAG}` under ``…/genai_mapping/eval/`` on the target
silver volume. ``SHOT_TAG`` is read from ``manifest.eval``.
"""
...
from ..manifest.eval import (
    MODELS,
    _eval_run_root,
    _require_uc_catalog_for_eval,
    _resolve_gold_transformation_map_for_eval,
...
def run():
    ...
    OUTPUT_DIR = EVAL_BASE / "eval_outputs_2b"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    gold_path = _resolve_gold_transformation_map_for_eval(target_id)
    GOLD_TM = load_json(str(gold_path))
    logger.info(f"Loaded gold transformation map from {gold_path}")

    manifest_path = _resolve_target_mapping_manifest_for_eval(target_id)
