...
Databricks job correlation; ``reference_id`` / ``reference_content_hash`` for the pinned
few-shot used during SMA onboard), ``pipeline_phases``, ``hitl_reviews``, ``reference_pins``.
...
def update_onboard_pipeline_run_reference(
    ...
    *,
    ...
    """
    Record which pinned few-shot SMA used for this onboard.

    ``reference_content_hash`` must be the pin's ``content_hash`` (from
    ``genai_reference_pin.json`` / ``reference_pins``), not a hash of school ``active/``.
    """
    c = str(catalog).strip()
    inst = str(institution_id).strip()
    rid = str(onboard_run_id).strip()
