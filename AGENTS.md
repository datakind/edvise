...
1. **Start Release** (`start-release.yml`, manual `workflow_dispatch` with no version input) infers the
   next version from conventional PR titles since the last tag (`feat` → minor, `type!` / `feat!` / `fix!`
   → major, otherwise patch), branches `release/<version>` off `develop`, runs
   `python -m edvise.scripts.update_version` to bump `pyproject.toml` and generate `CHANGELOG.md` entries,
   pushes the branch, then dispatches `release-branch-ci-dev`.
