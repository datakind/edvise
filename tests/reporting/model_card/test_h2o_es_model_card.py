...
def test_init_with_es_config(mock_config, mock_client):
    """Test initialization with ESProjectConfig."""
    card = H2OESModelCard(
        config=mock_config,
        catalog="catalog",
        model_name="my_model",
...
def test_get_plot_config(mock_config, mock_client):
    ...
    for plot_name in expected_plots:
        ...
        assert len(plot_config[plot_name]) == 4
...
def test_required_plot_artifacts_list():
    ...
    assert H2OESModelCard.REQUIRED_PLOT_ARTIFACTS == expected_artifacts
