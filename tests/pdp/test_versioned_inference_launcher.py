"""Unit tests for versioned inference launcher helpers."""
...
from edvise.runtime.versioned_inference import model_resolution as mm
from edvise.runtime.versioned_inference import runtime_compat as rc
from edvise.runtime.versioned_inference.tasks import validate as validate_task
...
def test_normalize_uc_model_short_name() -> None:
    ...
    assert (
        normalize_legacy_uc_model_short_name(
    ...
    assert normalize_legacy_uc_model_short_name("short_only") == "short_only"
...
def test_parse_python_xy() -> None:
    assert rc.parse_python_xy("3.11") == (3, 11)
    assert rc.parse_python_xy("nope") is None


def test_check_runtime_bundle_execution_mode_dab() -> None:
    ok, msg = rc.check_runtime_bundle_compatibility(
        {"execution_mode": "dab"},
...
def test_databricks_runtime_compatible_short_cluster_version() -> None:
    assert rc.databricks_runtime_compatible("15.4.x-cpu-ml-scala2.12", "15.4")
    assert rc.databricks_runtime_compatible(
        "15.4.x-cpu-ml-scala2.12", "15.4.x-cpu-ml-scala2.12"
    )
    assert not rc.databricks_runtime_compatible("15.4.x-cpu-ml-scala2.12", "14.3")
