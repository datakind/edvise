...
def test_append_mapping_override_event_roundtrip(tmp_path: Path) -> None:
    log_path = tmp_path / "mapping_override_log.json"
    ts = datetime(2026, 5, 4, 12, 0, 0, tzinfo=timezone.utc)
    ev = MappingOverrideEvent(
        timestamp=ts,
        overridden_by="reviewer_a",
        agent="schema_mapping_agent",
        ...
        override_task_run_id=None,
    )
    append_mapping_override_event(log_path, "inst_x", ev)
    loaded = read_pydantic_json(log_path, MappingOverrideLog)
    assert loaded.institution_id == "inst_x"
    assert len(loaded.events) == 1
    assert loaded.events[0].override_type == "manifest_mapping"
    assert loaded.events[0].target_field == "term"


def test_append_mapping_override_event_injects_task_run_id_from_env(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("DATABRICKS_TASK_RUN_ID", "tr-override")
    log_path = tmp_path / "mapping_override_log.json"
    ts = datetime(2026, 5, 4, 12, 0, 0, tzinfo=timezone.utc)
    append_mapping_override_event(
        log_path,
    ...
    loaded = read_pydantic_json(log_path, MappingOverrideLog)
    assert loaded.events[0].override_task_run_id == "tr-override"
