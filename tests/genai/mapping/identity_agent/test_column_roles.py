...
def test_fallback_assigns_other_for_missing_column():
    result = _roles(
        assignments=[
            ColumnRoleAssignment(
                column="pidm",
...
def test_missing_column_fallback_after_retry_exhausted():
    ...
    assert result.role_for("dev_engl") == ColumnRole.OTHER
