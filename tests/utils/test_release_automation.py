...
class TestPRFetching:
    ...
    def test_get_pr_numbers_no_since_tag(self, mock_run):
        ...
        mock_run.return_value = self._mock_git_log("Merge pull request #125 from branch\n")
    ...
    def test_get_pr_numbers_no_matches(self, mock_run):
        ...
        mock_run.return_value = self._mock_git_log("Some other commit message\nFixes issue #123\n")
        result = automate_releases.get_pr_numbers_from_git_log("v0.1.8")
        assert result == []
...
class TestUpdateChangelog:
    ...
    def test_update_changelog_new_version(self):
        ...
        try:
            with patch("edvise.scripts.update_version.automate_releases.get_pr_titles_since_last_version") as mock_get_prs:
                mock_get_prs.return_value = ["feat: new feature (#123)", "fix: bug fix (#124)"]
                
                update_version.update_changelog(changelog_path, "0.1.9", "owner/repo", "token")
    ...
    def test_update_changelog_duplicate_version(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            f.write("## 0.1.9 (2025-12-12)\n- Already exists\n\n## 0.1.8 (2025-12-11)\n- Old\n\n")
            changelog_path = Path(f.name)
        
        try:
            with patch("edvise.scripts.update_version.automate_releases.get_pr_titles_since_last_version"):
                update_version.update_changelog(changelog_path, "0.1.9", "owner/repo", "token")
                content = changelog_path.read_text()
    ...
    def test_update_changelog_empty_file(self):
        ...
        try:
            with patch("edvise.scripts.update_version.automate_releases.get_pr_titles_since_last_version") as mock_get_prs:
                ...
                update_version.update_changelog(changelog_path, "0.1.0", "owner/repo", "token")
...
class TestPRCreation:
    ...
    def test_check_existing_pr(self, mock_run):
        ...
        result = subprocess.run(
            ["gh", "pr", "list", "--head", "release/0.1.9", "--base", "main", 
    ...
    def test_create_backmerge_pr(self, mock_run):
        ...
        result = subprocess.run(
            [
                ...
                "--base", "develop",
                "--head", "main",
                "--title", f"chore: sync develop with release {version}",
                "--body", (
...
class TestWorkflowConditions:
    ...
    def test_branch_deletion_condition(self):
        ...
        should_trigger = (
            event_name == "pull_request" and
            merged and
            base_ref == "develop" and
            head_ref == "main" and
            title.startswith("chore: sync develop with release ")
        )
...
class TestWorkflowYAMLSyntax:
    ...
    def test_workflow_step_structure(self):
        ...
        for step in job["steps"]:
            assert "name" in step
            assert "uses" in step or "run" in step, f"Step '{step['name']}' missing 'uses' or 'run'"
